...
$sqlNew = "SET @idPath = (SELECT SUBSTR(JSON_SEARCH(friends, 'one', '$friend', NULL, '$.friends[*].id'), 2, LOCATE('id', JSON_SEARCH(friends, 'one', '$friend', NULL, '$.friends[*].id')) - 3) FROM `users` WHERE `usersId` = $id);
        UPDATE `users` SET friends=JSON_SET(friends, CONCAT(@idPath, '.request_type'), 'friends', CONCAT(@idPath, '.date'), CURDATE(), '$.friend_count', JSON_EXTRACT(friends, '$.friend_count') + 1) WHERE `usersId` = $id;";
$sqlNew .= "SET @idPathOther = (SELECT SUBSTR(JSON_SEARCH(friends, 'one', '$id', NULL, '$.friends[*].id'), 2, LOCATE('id', JSON_SEARCH(friends, 'one', '$id', NULL, '$.friends[*].id')) - 3) FROM `users` WHERE `usersId` = $friend);
        UPDATE `users` SET friends=JSON_SET(friends, CONCAT(@idPathOther, '.request_type'), 'friends', CONCAT(@idPath, '.date'), CURDATE(), '$.friend_count', JSON_EXTRACT(friends, '$.friend_count') + 1) WHERE `usersId` = $friend;";
